...
5. `data/`: muestras sinteticas pequenas y datos normalizados locales fuera de Git.
6. `evaluation`: metricas para comparar baseline y modelos evaluados con el mismo contrato.
...
Para la demo LoRA y la evaluacion del adapter, el proyecto usa Transformers + PEFT
directamente y no exporta el adapter a GGUF ni lo sirve mediante llama.cpp.
